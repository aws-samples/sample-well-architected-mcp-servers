...
async def test_orchestration():
    ...
    # Load utils modules
    logging_utils_path = os.path.join(base_dir, "agent_config", "utils", "logging_utils.py")
    logging_utils = load_module_from_path("logging_utils", logging_utils_path)
    
    error_handling_path = os.path.join(base_dir, "agent_config", "utils", "error_handling.py")
    error_handling = load_module_from_path("error_handling", error_handling_path)
    
    # Load connection pool
    connection_pool_path = os.path.join(base_dir, "agent_config", "orchestration", "connection_pool.py")
    ...
    print(f"✅ Created connection pool manager with max {pool_manager.max_connections_per_server} connections per server")
    ...
    print(f"✅ Pool stats: {stats['overall']['total_servers']} servers, {stats['overall']['total_connections']} connections")
