...
class CognitoSSMParameterGenerator:
    ...
    def get_stack_resources(self, stack_name: str) -> Dict[str, Any]:
        ...
        try:
            ...
            if e.response['Error']['Code'] == 'ValidationError':
                logger.error(f"Stack '{stack_name}' does not exist or is not accessible")
            else:
        ...
        try:
            ...
            try:
                ...
                if 'DomainDescription' in domain_response:
                    details['user_pool_domain'] = domain_response['DomainDescription']['Domain']
            except (ClientError, Exception) as e:
                # Domain might not exist or have different name - this is not critical
                logger.debug(f"Could not retrieve domain for user pool {user_pool_id}: {e}")
    ...
    def create_ssm_parameters(self, cognito_resources: Dict[str, str], 
                            stack_name: str, dry_run: bool = False) -> Dict[str, bool]:
        ...
        if 'user_pool_id' in cognito_resources:
            user_pool_details = self.get_user_pool_details(cognito_resources['user_pool_id'])
        ...
        for param_name, param_config in ssm_parameters.items():
            ...
            if dry_run:
                logger.info(f"[DRY RUN] Would create: {param_name} = {param_config['value']}")
    ...
    def generate_parameters_from_stack(self, stack_name: str, dry_run: bool = False) -> bool:
        ...
        try:
            ...
            if dry_run:
                logger.info(f"   🔍 This was a dry run - no parameters were actually created")
...
Examples:
  ...
  python generate_cognito_ssm_parameters.py --stack-name my-coa-stack --region us-west-2
    ...
    parser.add_argument(
        '--stack-name',
        type=str,
        help='CloudFormation stack name to scan'
    )
    ...
    parser.add_argument(
        '--validate',
        action='store_true',
        help='Validate existing SSM parameters'
    )
    ...
    try:
        ...
        if args.validate:
            ...
            if valid_count == total_count:
                ...
                logger.error(f"❌ {total_count - valid_count} parameters are invalid or missing")
                return 1
        
        elif args.stack_name:
            success = generator.generate_parameters_from_stack(args.stack_name, args.dry_run)
