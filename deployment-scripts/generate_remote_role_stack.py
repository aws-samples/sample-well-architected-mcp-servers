...
Examples:
  ...
  python generate_remote_role_stack.py --additional-policies arn:aws:iam::aws:policy/ReadOnlyAccess
    ...
    parser.add_argument(
        '--external-id',
        help='External ID for additional security in trust policy'
    )
...
def get_agentcore_runtime_role_arn(region: str) -> str:
    ...
    """
    logger.info(f"Retrieving AgentCore Runtime Role ARN from Parameter Store in region {region}")
        ...
        try:
            ...
            if agent_arn:
                ...
                try:
                    ...
                    logger.debug("Execution role ARN not found in parameter store, will derive from agent ARN")
                ...
                logger.warning("Note: You may need to specify the exact execution role ARN manually")
                return execution_role_arn
                
        except ClientError as e:
            if e.response['Error']['Code'] == 'ParameterNotFound':
                logger.debug("Connection info parameter not found, trying direct agent ARN parameter")
            else:
                raise
...
def generate_cloudformation_template(config: RemoteRoleConfig) -> Dict[str, Any]:
    ...
    logger.debug(f"Generated template parameters: {list(template['Parameters'].keys())}")
...
def generate_security_services_policy(config: RemoteRoleConfig) -> Dict[str, Any]:
    ...
    if config.custom_policy_statements:
        ...
        logger.debug(f"Added {len(config.custom_policy_statements)} custom policy statements")
    ...
    policy_resource = {
        ...
        'Properties': {
            ...
            'Roles': [
                {'Ref': 'CrossAccountMCPRole'}
            ]
        }
    }
    
    logger.debug(f"Generated security services policy with {len(policy_statements)} statements")
...
def generate_template_outputs(config: RemoteRoleConfig) -> Dict[str, Any]:
    ...
    outputs = {
        'RoleArn': {
            ...
            'Value': {
                'Fn::GetAtt': ['CrossAccountMCPRole', 'Arn']
            },
            'Export': {
                'Name': {
        ...
        'Value': {
            'Fn::Sub': [
                'To assume this role from AgentCore Runtime, use: aws sts assume-role --role-arn ${RoleArn} --role-session-name MCP-Session${ExternalIdParam}',
                {
                    ...
                    'ExternalIdParam': ' --external-id ' + config.external_id if config.external_id else ''
                }
            ]
        }
    }
...
def save_template(template: Dict[str, Any], output_dir: str, config: RemoteRoleConfig) -> str:
    """
...
def save_metadata(config: RemoteRoleConfig, output_dir: str, template_path: str) -> str:
    ...
    if config.external_id:
        metadata['deployment_instructions']['parameters']['ExternalId'] = config.external_id
...
def main():
    ...
    try:
        ...
        try:
            ...
            logger.error("1. Ensure the WA Security MCP deployment completed successfully")
            logger.error("2. Check that you have access to Parameter Store in the current AWS account")
            logger.error("3. Verify the deployment created parameters under /coa/components/wa_security_mcp/")
            raise e
        except Exception as e:
        ...
        if not validate_role_arn(runtime_role_arn):
            ...
            logger.error("The role ARN should be in format: arn:aws:iam::account:role/RoleName")
        ...
        try:
            ...
            logger.warning(f"Failed to save metadata (template generation still successful): {e}")
        ...
        print(f"     --stack-name remote-mcp-role \\")
        print(f"     --capabilities CAPABILITY_IAM \\")
        print(f"     --profile target-account-profile")
        print("="*60)
